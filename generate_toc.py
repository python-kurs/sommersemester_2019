...
if __name__ == '__main__':
    ...
    for ifolder, _, ifiles in os.walk(args.content_folder):
        ...
        if any(any(ifile.endswith(ii) for ii in [".ipynb", ".md"]) for ifile in ifiles):
            ...
            if path_rel_to_content != "":
                toc.append({"title": path_rel_to_content[1:], "url": os.path.join(path_rel_to_content, path_rel_to_content[1:]), "sections": []})
                ifiles.remove(path_rel_to_content[1:] + ".md")
        ...
        for ifile in ifiles:
            ...
            if path_rel_to_content != "":
                toc[-1]["sections"].append({'title': i_title, 'url': i_url})
            else:
